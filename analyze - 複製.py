...
from database import get_db_client, get_db_path # 引入統一的資料庫工具
...
def tool_get_hot_stocks(db_path: str) -> str:
    """
...
def tool_summarize(stock_id: str, hot_signal: str, conference: str) -> str:
    ...
    positive_words = ["growth", "record", "strong", "increase",
                      "exceed", "成長", "創新高", "強勁"]
    negative_words = ["decline", "weak", "headwind", "cautious",
                      "cut", "下修", "庫存", "保守"]
    
    combined = hot_signal + conference
    ...
    return f"{company_name}（{stock_id}）綜合判斷：{verdict}（正面訊號 {pos} 個，負面訊號 {neg} 個）"
...
def run_agent():
    ...
    # Step 1：找今日有訊號的股票
    print("\n[Step 1] 掃描今日市場訊號...")
    hot_signal = tool_get_hot_stocks(get_db_path()) # 從統一設定取得路徑
    print(hot_signal)
